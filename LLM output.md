  ...
  <script>
    ...
    function parseLapFile(text, filename) {
      ...
      if (idx.time === -1 || idx.dist === -1) {
        ...
        throw new Error('Could not find Time or Distance columns. Adjust guessColumnIndices() to match your header names.');
      }
    ...
    function getSelectedLaps() {
      const checkedInputs = Array.from(lapList.querySelectorAll('input[type="checkbox"]:checked'));
    ...
    async function handleFiles(files) {
      ...
      if (loaded === 0 && failed > 0) {
        setStatus('Failed to parse all files. Check console for details and adjust header mappings.');
      } else {
        const msg = [];
