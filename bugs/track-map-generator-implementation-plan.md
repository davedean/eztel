...
- Calibration lap quality (driver going off-track)
  - *Mitigation*: Outlier detection, manual override in preview tool
- Coordinate system mismatch (X/Y vs X/Z)
  - *Mitigation*: Test with existing lap renderer, ensure transform consistency
- Closed-loop geometry (start/end alignment)
  - *Mitigation*: Circular smoothing, closure validation in QA
